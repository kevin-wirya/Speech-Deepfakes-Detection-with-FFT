...
\label{fig}
\end{figure}
Geometric distance calculation is used for confidence classification. It is derived from the formula mentioned in the theoretical framework. It handles errors and also division by zero. In addition, we use weights approach to compute the distance in order to maximize the usage of all three metrices.

\begin{figure}[htbp]
\centerline{\includegraphics[width=8cm]{img/code_predict.png}}
\caption{Classification Based on Geometric Distance}
\small \centering (Source: Author)
\label{fig}
\end{figure}
